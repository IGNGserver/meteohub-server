...
describe("HTTP contract", () => {
  it("requires auth, supports pairing, location CRUD and tombstone sync", async () => {
    ...
    const openapi = await app.inject({ method: "GET", url: "/api/v1/openapi.json" }); expect(openapi.statusCode).toBe(200); expect(openapi.json().paths["/api/v1/locations/{id}/forecast"]).toBeDefined();
  });
